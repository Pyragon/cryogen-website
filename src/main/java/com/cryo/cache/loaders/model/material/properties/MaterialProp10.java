...
public class MaterialProp10 extends MaterialProperty {
    ...
    public void decode(int i_1, InputStream stream) {
        if (i_1 == 0) {
            ...
            if (i_4 == 0) {
                anIntArrayArray9822 = new int[stream.readUnsignedByte()][4];

                for (int i_5 = 0; i_5 < anIntArrayArray9822.length; i_5++) {
                    anIntArrayArray9822[i_5][0] = stream.readUnsignedShort();
                    anIntArrayArray9822[i_5][1] = stream.readUnsignedByte() << 4;
                    anIntArrayArray9822[i_5][2] = stream.readUnsignedByte() << 4;
                    anIntArrayArray9822[i_5][3] = stream.readUnsignedByte() << 4;
                }
            } else {
                method15269(i_4);
            }
