	...
	private void readValues(InputStream stream, int opcode) {
		if (opcode == 1) {
			...
			for (int i_5_ = 0; i_5_ < i_4_; i_5_++) {
				types[i_5_] = (byte) stream.readByte();
			...
			transformTo = new int[i_16_ + 2];
			for (int i_17_ = 0; i_17_ <= i_16_; i_17_++)
				transformTo[i_17_] = stream.readBigSmart();
			transformTo[1 + i_16_] = i_15_;
		} else if (78 == opcode) {
...
//			}
	...
	void method7966() {
		if (interactable == -1) {
			interactable = 0;
			if (null != types && types.length == 1 && (types[0] == 10))
				interactable = 1;
