...
import com.cryo.entities.logs.BOB;
...
@EndpointSubscriber
public class BOBLogs {

    @Endpoint(method = "POST", endpoint = "/logs/bob/load")
    public static String renderDeathLogs(Request request, Response response) {
        Account account = AccountUtils.getAccount(request);
        if(account == null) return com.cryo.modules.account.Login.renderLoginPage("/logs/bob", request, response);
        ...
        model.put("title", "BOB Logs");
        model.put("module", "/logs/bob");
        model.put("moduleId", "bob");
        model.put("info", new ArrayList<String>() {{
            add("The following page lists all in-game transfers between BOBs by players on Cryogen.");
    ...
    @Endpoint(method = "POST", endpoint = "/logs/bob/table")
    public static String renderTable(Request request, Response response) {
        ...
        Object[] condition = ListManager.getCondition(filterValues, BOB.class, false);
        if(condition.length == 2 && condition[0] != null) {
            query += (String) condition[0];
            values.addAll((ArrayList<Object>) condition[1]);
        }
        int total = getConnection("cryogen_logs").selectCount("bob", query, values.toArray());
        String order = ListManager.getOrder(model, sortValues, BOB.class, page, total, false);
        List<BOB> bobs = getConnection("cryogen_logs").selectList("bob", query, order, BOB.class, values.toArray());
        if(bobs == null)
            return error("Error loading bobs. Please try again.");
        ListManager.buildTable(model, "logs", bobs, BOB.class, account, sortValues, filterValues, false);
