...
import com.cryo.entities.logs.GrandExchange;
...
@EndpointSubscriber
public class GrandExchangeLogs {

    @Endpoint(method = "POST", endpoint = "/logs/ge/load")
    public static String renderDeathLogs(Request request, Response response) {
        Account account = AccountUtils.getAccount(request);
        if(account == null) return com.cryo.modules.account.Login.renderLoginPage("/logs/ge", request, response);
        ...
        model.put("title", "Grand Exchange Logs");
        model.put("module", "/logs/ge");
        model.put("moduleId", "ge");
        model.put("info", new ArrayList<String>() {{
            add("The following page lists all in-game grand exchange actions made by players on Cryogen.");
    ...
    @Endpoint(method = "POST", endpoint = "/logs/ge/table")
    public static String renderTable(Request request, Response response) {
        ...
        Object[] condition = ListManager.getCondition(filterValues, GrandExchange.class, false);
        if(condition.length == 2 && condition[0] != null) {
            query += (String) condition[0];
            values.addAll((ArrayList<Object>) condition[1]);
        }
        int total = getConnection("cryogen_logs").selectCount("grand_exchange", query, values.toArray());
        String order = ListManager.getOrder(model, sortValues, GrandExchange.class, page, total, false);
        List<GrandExchange> deaths = getConnection("cryogen_logs").selectList("grand_exchange", query, order, GrandExchange.class, values.toArray());
        if(deaths == null)
            return error("Error loading grand exchange actions. Please try again.");
        ListManager.buildTable(model, "logs", deaths, GrandExchange.class, account, sortValues, filterValues, false);
