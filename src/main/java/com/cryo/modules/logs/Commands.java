...
public class Commands {
    ...
    @Endpoint(method = "POST", endpoint = "/logs/command/load")
    public static String renderCommandLogs(Request request, Response response) {
        ...
        model.put("title", "Command Logs");
        model.put("module", "/logs/command");
        model.put("moduleId", "command");
        model.put("info", new ArrayList<String>() {{
            add("The following page lists all commands made in-game by players on Cryogen.");
    ...
    @Endpoint(method = "POST", endpoint = "/logs/command/table")
    public static String renderTable(Request request, Response response) {
        ...
        Object[] condition = ListManager.getCondition(filterValues, Command.class, false);
        if(condition.length == 2 && condition[0] != null) {
            query += (String) condition[0];
            values.addAll((ArrayList<Object>) condition[1]);
        }
        int total = getConnection("cryogen_logs").selectCount("command", query, values.toArray());
        String order = ListManager.getOrder(model, sortValues, Command.class, page, total, false);
        List<Command> commands = getConnection("cryogen_logs").selectList("command", query, order, Command.class, values.toArray());
        if(commands == null)
            return error("Error loading commands. Please try again.");
        ListManager.buildTable(model, "logs", commands, Command.class, account, sortValues, filterValues, false);
