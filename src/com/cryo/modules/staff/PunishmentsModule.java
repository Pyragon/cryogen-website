...
public class PunishmentsModule {
	...
	public static ArrayList<Comment> getComments(int id) {
		...
		Object[] data = PunishmentsConnection.connection().handleRequest("get-comments", id, 1);
		if (data == null)
			return comments;
	...
	public static Properties handleRequest(String action, Request request, Response response, Properties prop, WebModule module) {
		...
		switch (action) {
			case "search":
				...
				//Properties search_results = Website.instance().getSearchManager().search("punish", text, page, PunishmentsConnection.connection());
//				prop.put("success", search_results.get("success"));
//				if(!(boolean) search_results.get("success")) {
//					prop.put("error", search_results.get("error"));
//					break;
//				}
//				List<Punishment> punishments = (List<Punishment>) search_results.get("results");
...
//				Punishment punish = appeal ? new PunishUtils().getPunishmentFromAppeal(id) : new PunishUtils().getPunishment(id);
//				if (punish == null) {
...
//				prop.put("html", html);
			...
			case "create":
				...
				Punishment punish = null;//ew Punishment(-1, player, (type.equals("mute") ? 0 : 1), null, expiry, username, reason, true, 0);
				Object[] data = PunishmentsConnection.connection().handleRequest("create-punishment", punish);
				if(data == null) {
					prop.put("success", false);
...
//				} else
//					expiry = null;
//				PunishmentsConnection.connection().handleRequest("extend-punishment", expiry, id);
//				PunishmentsConnection.connection().handleRequest("add-comment", "cryobot", id, 1, "$for-name="+username+"$end has extended this punishment "+(expires == 0 ? "permanently" : "by "+expires+" days")+".");
...
//				prop.put("success", true);
//				int total = PunishUtils.getTotalPages(PunishmentsConnection.connection(), "punishments"+(archived ? "-a":""));
