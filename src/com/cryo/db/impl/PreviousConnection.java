...
public class PreviousConnection extends DatabaseConnection {
	...
	public String toString(HashMap<String, PreviousIP> list) {
		...
		for(Iterator<PreviousIP> it = list.values().iterator(); it.hasNext();) {
			PreviousIP previous = it.next();
			builder.append(previous.getIp());
			builder.append(", ");
			builder.append(previous.getDate().getTime());
			if(it.hasNext())
				builder.append(", ");
		}
	...
	public Object[] handleRequest(Object... data) {
		...
		case "add-new-ip":
			...
			if(list.containsKey(ip)) return null;
			list.put(ip, previous);
	...
	public static List<PreviousIP> getSorted(String username) {
		...
		previous.sort((v1, v2) -> { return (int) (v1.getDate().getTime() - v2.getDate().getTime()); });
		return previous;
	}
